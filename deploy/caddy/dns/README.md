...
to `RIFT_CADDY_DNS_PLUGINS` and run `tools/rift-ops release caddy`. If the plugin is
...
`tools/rift-ops release caddy --validate` to check the snippet parses before deploying.
