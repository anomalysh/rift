#!/usr/bin/env bash
# tools/backup.sh — back up the rift production stack.
#
...
# Default to the base compose file only: it defines the postgres service without
# the prod overlay's required `RIFT_TLS_MODE:?` variable, so a cron environment
...
# Resolve DB user/name. RIFT_POSTGRES_DSN, when set, overrides them; otherwise
# take the values the container was initialised with.
DB_USER=""
DB_NAME=""
...
[ -n "$DB_USER" ] && [ -n "$DB_NAME" ] || die "could not determine database user/name"
...
# rift backup MANIFEST -- generated by tools/backup.sh, do not edit.
